...
\begin{itemize}
\item \textbf{Waheed et al.\ [Belle], PRD 100, 052007 (2019)} --- $D^0$
  mass window, narrow $\Delta m$ (slow-pion) window, Fox--Wolfram $R_2$
  for continuum suppression, $e/\mu$ channels treated separately and
  combined.
\item \textbf{BaBar, PRD 88, 072012 (2013)} --- $\mmiss$ as the primary
  discriminant between single-neutrino ($D^{*}\ell\nu$) and
  multi-neutrino ($D^{*}\tau\nu$) final states.
\item \textbf{Caria et al.\ [Belle], PRL 124, 161803 (2020)} and
  \textbf{Belle~II, PRD 110, 072020 (2024)} --- simultaneous $e/\mu$
  template fit sharing one signal strength.
\end{itemize}
...
Following Waheed et al., a genuine $D^0$ candidate and a genuine slow
pion from $D^{*}\to D^0\pi$ are required:
\begin{itemize}
...
\item No experimental or physics-modeling systematic uncertainty is
  ...
  summing in quadrature to a few percent [Belle~II, arXiv:2210.13143];
  the Belle~II hadronic-tag $R(D^{*})$ analysis separately reports
  $\approx$2.1\% from hadronic-$B$-decay modeling and $\approx$2.0\% from
  fit-template PDF-shape systematics [Adachi et al., PRD 110, 072020
  (2024), arXiv:2401.02840]. Taking these together as representative of
...
\item \textbf{No fake-$D^{*}$ combinatorial background is modeled} in
  ...
  Waheed et al.\ could not be reproduced here; a full detector simulation
